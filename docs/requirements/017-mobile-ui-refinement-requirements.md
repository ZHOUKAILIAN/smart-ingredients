...
### 需求 9: 偏好选择 Modal 高度适配

- **编号**: FR-017-009
- **优先级**: 高
- **描述**: 调整偏好选择 Modal 在不同屏幕尺寸下的高度,避免在小屏设备上占据过多空间
- **影响页面**: 首页/拍照页 (`capture.rs`)
- **验收标准**:
  - [ ] Modal 在小屏设备上高度不超过视口的 85%
  - [ ] Modal 在超小屏设备上高度不超过 75%
  - [ ] Modal 内容在不同机型上显示比例协调
  - [ ] 内容过多时可以滚动查看
  - [ ] 标题、说明文字大小适中,清晰可读
...
- **编号**: FR-017-010
- **优先级**: 高
- **描述**: 修复滚动区域划分,确保只有内容区域可滚动,顶部和底部固定区域不参与滚动
- **影响页面**: 所有页面
- **验收标准**:
  - [ ] 顶部状态栏留白区域固定,不参与滚动
  - [ ] 底部导航栏固定,不参与滚动
...
- **编号**: FR-017-011
- **优先级**: 中
- **描述**: 调整首页按钮和步骤说明的尺寸,使整体视觉更精致协调
- **影响页面**: 首页/拍照页 (`capture.rs`)
- **验收标准**:
  - [ ] "开始分析"按钮高度调整为 48-50px
  - [ ] 步骤图标大小调整为 50px 左右
  - [ ] 步骤说明文字大小适中(标题 14px,说明 12px)
  - [ ] 整体布局更紧凑,视觉层次清晰
  - [ ] 在小屏设备上进一步缩小尺寸
...
- **验收标准**:
  ...
  - [ ] 所有按钮在不同机型上尺寸一致
  - [ ] 按钮触摸区域符合规范(≥ 44x44 dp)
  - [ ] 视觉层次清晰(主按钮 > 次要按钮 > 链接按钮)

### 需求 13: 顶部背景对比度优化

- **编号**: FR-017-013
- **优先级**: 高
- **描述**: 优化顶部状态栏区域背景色,确保系统状态栏文字在所有主题下清晰可见
- **影响页面**: 所有页面(全局样式)
- **验收标准**:
  - [ ] 顶部状态栏区域背景不透明或半透明
  - [ ] 系统状态栏文字(时间、电量等)清晰可见
...
- **编号**: FR-017-014
- **优先级**: 中
- **描述**: 调整底部导航栏高度和图标文字尺寸,减少屏幕空间占用
- **影响页面**: 所有页面(底部导航栏)
- **验收标准**:
  - [ ] 导航栏高度调整为 45px (从 68px)
  - [ ] 图标大小调整为 20px (从 22px)
  - [ ] 文字大小约 11px,保持清晰可读
  - [ ] 导航栏在不同机型上高度一致
  - [ ] 视觉层次清晰,不拥挤
...
| 1.1 | 2026-01-31 | Claude + User | 新增功能需求 FR-017-009 至 FR-017-014 (6个具体优化问题) |
